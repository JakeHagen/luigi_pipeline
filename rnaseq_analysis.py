...
output_dir = '/sc/orga/projects/argmac01a/luigi_rna'
...
class output_dir(luigi.Task):
	...
	def output(self):
		return luigi.LocalTarget('%s' % output_dir)

class star_align(luigi.Task):
	sample = luigi.Parameter()
	path = luigi.Parameter()
	def requires(self):
		return index_STAR_genome()#, output_dir()

	def run(self):
		if not os.path.exists('%s/%s/star' % (output_dir, self.sample)):
			os.makedirs('%s/%s/star' % (output_dir, self.sample))
		...
		s_command = [
				'STAR',
					...
					'--outFileNamePrefix %s/%s/star/%s.' % (output_dir, self.sample, self.sample)
					]
		star = subprocess.Popen(s_command)
		star.wait()
		if star.returncode != 0:
			subprocess.call(['rm', '-rf', '%s/%s/star' % (output_dir, self.sample)])
	def output(self):
		return luigi.LocalTarget('%s/%s/star/%s.Aligned.out.bam' % (output_dir, self.sample, self.sample))
